...
def flash_bwd_kernel(
    ...
    for i in range(tl.cdiv(N_QUERIES, Q_TILE_SIZE)):
        ...
        dv = tl.dot(p.T.to(tl.bfloat16), do.to(tl.bfloat16), acc=dv)

        # dp = do @ v.T -> (QTS, D) @ (D, KTS) = (QTS, KTS)
        dp = tl.dot(do.to(tl.bfloat16), v.T.to(tl.bfloat16))
        # ds = p * (dp - D) * scale
        ds = p * (dp - d[:, None]) * scale

        # _dq = ds @ k.T -> (QTS, KTS) @ (KTS, D) = (QTS, D)
        # Note: k is already (D, KTS), so k.T is (KTS, D)
        _dq = tl.dot(ds.to(tl.bfloat16), k.T.to(tl.bfloat16))
        ...
        dk = tl.dot(ds.T.to(tl.bfloat16), q.to(tl.bfloat16), acc=dk)
    ...
    tl.store(dK_block_ptr, dk.to(tl.bfloat16), boundary_check=(0, 1))
    tl.store(dV_block_ptr, dv.to(tl.bfloat16), boundary_check=(0, 1))
