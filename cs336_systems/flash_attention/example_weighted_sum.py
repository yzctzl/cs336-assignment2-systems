# ruff: noqa: F841
# pyright: reportUnusedFunction=false, reportUnusedVariable=false, reportUnreachable=false
import torch
import triton
...
class WeightedSumFunc(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor, weight: torch.Tensor):
        ...
        ctx.D_TILE_SIZE = max(1, triton.next_power_of_2(D) // 16)  # Roughly 16 loops through the embedding dimension
        ...
        weighted_sum_fwd[(triton.cdiv(n_rows, ctx.ROWS_TILE_SIZE),)](
            ...
            weight.stride(0),
            y.stride(0),  # y is flattened, stride is always 1
            ROWS=n_rows, D=D,
            ROWS_TILE_SIZE=ctx.ROWS_TILE_SIZE, D_TILE_SIZE=ctx.D_TILE_SIZE,  # pyright: ignore[reportArgumentType]
        )
    ...
    def backward(ctx, grad_out: torch.Tensor):
        ...
        weighted_sum_backward[(triton.cdiv(n_rows, ROWS_TILE_SIZE),)](
            ...
            x.stride(0), x.stride(1),
            weight.stride(0),
            grad_out.stride(0),  # grad_out is flattened, stride is always 1
...
x_input = torch.randn(4, 32, device="cuda", requires_grad=True)
weight_input = torch.randn(32, device="cuda", requires_grad=True)

output = f_weightedsum(x_input, weight_input)
